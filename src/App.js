...
import SoundComponent from './components/playSound';
...
class App extends Component {
  ...
  render() {
    ...
    const timeOptions = this.state.timeValues.map((duration) =>
      <button key={duration}  className={ !this.state.timeHovered && duration === this.state.desiredTime 
                                          ? "active" : "" } onClick={ () => {this.timeSelect({duration})} }>{duration/60} Minutes</button>
    );

    const audioOptions = this.state.audioNames.map((audioName) =>
      <button key={audioName}   className={ !this.state.audioHovered && this.state.audioUrl === "audio/" + audioName.toLowerCase() + ".mp3" 
